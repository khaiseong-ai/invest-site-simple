...
// 放在文件顶部或这个块前面：Vercel 后端地址
const API_BASE = 'https://invest-site-simple.vercel.app';
...
$("#emailForm")?.addEventListener("submit", async (e) => {
  ...
  try {
    const resp = await fetch(`${API_BASE}/api/send-login-link`, {
      method: "POST",
    ...
    if (ct.includes("application/json")) {
      ...
      data = { ok: false, error: await resp.text() };
    }
